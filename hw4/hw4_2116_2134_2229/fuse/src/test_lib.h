...
char* compressBuffer(char* buf, FILE* hash_file, char* dirPath, int size, int base){
    char* output;
    char block[base+1];
    char char_id[5];
    int nofBlocks = 0;
...
{
    // returns the containing Bytes of the specified block //
    FILE *block_file;
    char *block_contents;
    char *block_file_name;

    fprintf(diagn_fd, "In readBlock\n");                                      //diagnostics
    ...
    fprintf(diagn_fd, "Out readBlock\n");                                      //diagnostics
...
char *readFile( char *dirPath, size_t size, off_t offset, FILE *file, int base, char* fpath )
{
    ...
    if ( no_blocks == 0 || strncmp(fpath, dirPath, 79) == 0 )
    {
       return NULL;
    }
...
char* getOverwrittenBlocks( char* path, int size, int offset, int base ){
    FILE * fd = fopen(path, "r+");
    char *str;
    int skip = getNewOffset(offset, base);             //mul by 4 because every ID has "XXX\n"
    int bytesRead = 0;
    ...
    log_msg("...skip=->%d<-/size=->%d<-/ BytesRead=->%d<-\n", skip, size, bytesRead);
    str[bytesRead] = '\0';
    return(str);
}
...
int removeBlocks( char* input, FILE *hash_file,char* dirPath ){
    ...
    for ( j = 0; j < nofIDs; j++ ){                        //For all the IDs in the string
        ...
        log_msg("...ID=->%d<-/counter=->%d<-/ nofIDs=->%d<-\n", IDToRemove, counter, nofIDs);
        // Delete file if there is no more oinks to this block //
        if ( counter == 0 )
        {
