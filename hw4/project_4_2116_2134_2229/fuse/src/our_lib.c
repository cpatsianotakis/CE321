...
void createBlock(char* block, int id,char* dirPath ,FILE * diagn_fd ){   
    ...
    fputs("Out create_block\n", diagn_fd);                   //diagnostic MSG
...
int compareHash( int *isNewBlock, FILE *hash_file, char* in_hash,FILE * diagn_fd ){
    ...
    *isNewBlock = 0;                             //flag for creating block
    ...
    if (found){
        ...
        fseek( hash_file, -3,SEEK_CUR);                    // Read counter value //
        fread (counter_char, sizeof(char), 2, hash_file);
        counter_char[2] = '\0';
        counter = atoi ( counter_char );
        
        if ( counter == 0)                                 // Check if new file block must be created //
            *isNewBlock = 1;

        fseek( hash_file, -2,SEEK_CUR);                    // Upgrade counter value //
...
int PushBlock(char* block,FILE *hash_file,char* dirPath, FILE * diagn_fd){
    ...
    if (isNewBlock){
        ...
        createBlock(block,id, dirPath, diagn_fd);
    }
    else{
...
char *readBlock( char *dirPath, int id, int base, FILE * diagn_fd)
{
    ...
    if ( block_file_name == NULL ) {
        fprintf(diagn_fd, "Error in block file name\n");
    ...
    if ( block_contents == NULL ) {
        fprintf(diagn_fd, "Error in malloc for block contents!\n");
        return NULL;
    }
...
char *readFile( char *dirPath, size_t size, off_t offset, FILE *file, int base, char* fpath )
{
    ...
    int i;
    int id;
    char diagnosticsPath[PATH_MAX];
    char *read_result, *new_block;
    char id_char[5];

    sprintf(diagnosticsPath,"%s/Diagnostics.txt",dirPath);
    FILE * diagn_fd = fopen(diagnosticsPath,"r+");
    fseek(diagn_fd, 0, SEEK_END);
    fprintf(diagn_fd, "------------\\/------------\n");                             //diagnostics
    fprintf(diagn_fd, "--------------------------\n");                              //diagnostics
    ...
    //This if statement is need only if you want to read a block 
    //file inside mountdir/.Storage directory
