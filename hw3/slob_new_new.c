...
// static int find_min_block( struct page *sp, size_t size, int align, slob_t *best_block  )
// {
...
// 	slobidx_t avail = slob_units( min_block );
// 	next = slob_next(min_block);
// 	if (avail == units) { /* exact fit? unlink. */
// 		if (min_block_prev)
// 			set_slob(min_block_prev, slob_units(min_block_prev), next);
...
static void *slob_alloc(size_t size, gfp_t gfp, int align, int node)
{
	...
	while ( b == NULL ) 
	{
		...
		list_for_each_entry(sp, slob_list, list) {
			...
			if (  (sp->list.prev)->next == black_list_head || (sp->list.prev)->prev != slob_list->prev)
				break;
