...
static void *slob_alloc(size_t size, gfp_t gfp, int align, int node)
{
...
#ifdef BEST_FIT_ALLOCATOR
	struct list_head *prev, *next, *cur;
	struct page *best_pg;
#endif
#ifdef DEFAULT_ALLOCATOR
	...
	while ( b == NULL )
	{
		...
		if ( slob_list -> next == slob_list )
		{
			// Remake the chain //
			for ( cur = slob_list -> prev, next = slob_list; cur != slob_list; next = cur, cur = cur -> prev )
			{
				cur -> next = next;		
			}
			break;
		...
		list_for_each_entry(sp, slob_list, list) {
			...
			if (sp->units < SLOB_UNITS(size)) 
			{				
				for ( prev = sp->list.prev; prev -> next != NULL; prev = prev -> prev);
				next = sp -> next;

				prev -> next = next;
				sp -> next = NULL;
				sp = __container_of(prev, sp, list_head);
				continue;
			}
		...
		if ( !b ) 
		{
			for ( prev = best_pg.list.prev; prev -> next != NULL; prev = prev -> prev);
				next = best_pg -> next;

				prev -> next = next;
				best_pg -> next = NULL;
				best_pg = __container_of(prev, best_pg, list_head);
		}
